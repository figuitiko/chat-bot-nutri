-- Repair module and step sortOrder for the Nutri course.
-- Identifies the course by unique step slugs — safe even if the course slug/name was edited.
-- Uses inline VALUES aliases to avoid unnamed-column CTE ambiguity.

WITH nutri_course AS (
  SELECT DISTINCT c.id
  FROM "Course" c
  JOIN "CourseModule" cm ON cm."courseId" = c.id
  JOIN "CourseStep" cs ON cs."moduleId" = cm.id
  WHERE cs."slug" IN (
...
UPDATE "CourseModule" cm
SET "sortOrder" = m.sort_order
FROM nutri_course nc
CROSS JOIN (
  VALUES
    ('bienvenida', 1),
    ('modulo-1',   2),
    ...
    ('modulo-3',   4),
    ('modulo-4',   5)
) AS m(slug, sort_order)
WHERE cm."courseId" = nc.id
  AND cm."slug" = m.slug;
...
UPDATE "CourseStep" cs
SET "sortOrder" = s.sort_order
FROM nutri_course nc
JOIN "CourseModule" cm ON cm."courseId" = nc.id
CROSS JOIN (
  VALUES
...
) AS s(module_slug, step_slug, sort_order)
WHERE cs."moduleId" = cm.id
  AND cm."slug" = s.module_slug
  AND cs."slug" = s.step_slug;
