-- Repair persisted module and step order for the Nutri course even if the course slug/name
-- was edited in the dashboard. We identify the target course(s) by the unique training step slugs.
...
SET "sortOrder" = module_order.column2
FROM target_courses tc,
     module_order
WHERE cm."courseId" = tc.id
  AND cm."slug" = module_order.column1;
...
step_order(module_slug, step_slug, sort_order) AS (
  VALUES
...
JOIN "CourseModule" cm
  ON cm."courseId" = tc.id
JOIN step_order
  ON step_order.module_slug = cm."slug"
